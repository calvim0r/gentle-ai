...
| Kiro | `kiro` | Yes | Yes | Yes | No | No | `~/.kiro` |
...
| Feature | Claude Code | OpenCode | Gemini CLI | Cursor | VS Code Copilot | Kiro |
|---------|:-----------:|:--------:|:----------:|:------:|:---------------:|:----:|
| SDD orchestrator | Yes | Yes | Yes | Yes | Yes | Yes |
| Single-mode SDD | Yes | Yes | Yes | Yes | Yes | Yes |
| Multi-mode SDD | — | Yes | — | — | — | — |
...
- **Kiro** is an AI-native IDE. Skills are stored under `~/.kiro/skills/`, steering files (system prompt) under `~/.kiro/steering/`, and MCP config at `~/.kiro/settings/mcp.json`.
